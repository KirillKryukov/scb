...
my $new_temp_dir = "$temp_dir/$name-jarvis-c-dir";
...
if (!$wrapper_only)
{
    run("JARVIS -l $level '$name' >/dev/null 2>&1");
}
...
if ($wrapper_only)
{
    move($temp_sequence, "$out.jc");
}
else
{
    my $temp_compressed = "$temp_sequence.jc";
    if (!-e $temp_compressed) { die "Can't find \"$temp_compressed\"\n"; }
    move($temp_compressed, "$out.jc");
