...
my $new_temp_dir = "$temp_dir/$name-dir";
...
if (!$wrapper_only)
{
    run("ulimit -s unlimited; dna-compact n -$level '$name' >/dev/null 2>&1");
}
...
{
    my $temp_dcom_1 = "$temp_sequence.fp";
    if (!-e $temp_dcom_1) { die "Can't find \"$temp_dcom_1\"\n"; }
    my $temp_dcom_2 = "$temp_sequence.fp.txt.second";
    if (!-e $temp_dcom_2) { die "Can't find \"$temp_dcom_2\"\n"; }
    move($temp_dcom_1, "$out.fp");
    move($temp_dcom_2, $out);
