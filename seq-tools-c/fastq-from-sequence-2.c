...
static int name_number_width = 0;
...
static inline void print_name(unsigned long long n)
{
    //if (name_prefix == NULL) { fprintf(stdout, "@%llu\n", seq_number); }
    //else { fprintf(stdout, "@%s%llu\n", name_prefix, seq_number); }

    fputc('@', stdout);
    if (name_prefix != NULL) { fputs(name_prefix, stdout); }
    if (name_number_width > 0) { fprintf(stdout, "%0*llu\n", name_number_width, n); }
...
int main(int argc, char **argv)
{
    ...
    for (int i = 1; i < argc; i++)
    {
        if (i < argc - 1)
        {
            ...
            if (!strcmp(argv[i], "--name-number-width")) { i++; name_number_width = atoi(argv[i]); continue; }
